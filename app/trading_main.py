...
def main():
    ...
    try:
        ...
        app.processEvents()
        model = TradingModel(app_manager)
        ...
        all_runs_df = model.load_all_trading_runs(progress_callback=update_progress)
        
        progress.setLabelText("사용자 인터페이스를 구성하는 중입니다...")
        progress.setValue(85)
        app.processEvents()
        view = TradingView(app_manager)
        
        progress.setLabelText("컨트롤러를 초기화하는 중입니다...")
        progress.setValue(95)
        app.processEvents()
        # Controller는 Model을 통해 AppManager에 접근하므로 직접 전달할 필요 없음
        controller = TradingController(view, model) 
