...
class TradingController:
    ...
    def load_initial_data(self):
        ...
        try:
            ...
            all_runs_df = self.model.load_all_trading_runs(progress_callback=update_progress)
    ...
    def on_run_search(self):
        ...
        try:
            filtered_df = self.model.search_trading_runs(search_text)
    ...
    def on_run_selected(self):
        ...
        try:
            ...
            trading_date = pd.to_datetime(run_data['trading_date']).date()
            self.model.set_selected_run(model_id, trading_date)


            # 데이터 로딩
            self.loading_dialog.setLabelText("일별 성능 및 종목 요약 로딩 중...")
            # trading_performance는 특정 run(model_id+date)이 아닌, model 전체 기간으로 조회
            model_runs = self.model.all_trading_runs[self.model.all_trading_runs['model_id'] == model_id]
            start_date = model_runs['trading_date'].min()
            end_date = model_runs['trading_date'].max()

            performance_df = self.model.load_performance_data(model_id, start_date, end_date)
            traded_stocks_df = self.model.load_traded_stocks_summary(trading_date)
            # 뷰 업데이트
            self.view.update_performance_list(performance_df, model_id)
