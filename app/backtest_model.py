...
class BacktestModel(QAbstractTableModel):
    ...
    #     return self.backtest_manager._load_stock_info_map()

    def load_all_backtest_runs(self, progress_callback=None):
        if progress_callback: progress_callback(25, "백테스트 실행 정보를 조회하는 중입니다...")
        self.all_backtest_runs = self.app_manager.get_backtest_runs()
        if progress_callback: progress_callback(70, "백테스트 실행 목록을 로딩하는 중입니다...")
        return self.all_backtest_runs

    def _get_strategy_params(self, run_id: int):
        # 이 메서드는 all_backtest_runs를 사용하므로 AppManager 호출 필요 없음 (기존 로직 유지)
        if run_id not in self.run_strategy_params:
            ...
            if not run_info.empty:
                ...
                self.run_strategy_params[run_id] = {
                    'daily': {'strategy_name': row.get('strategy_daily', ''), **row.get('params_json_daily', {})},
                    'minute': {'strategy_name': row.get('strategy_minute', ''), **row.get('params_json_minute', {})}
                }
    ...
    def set_selected_run_id(self, run_id: int):
        if self.current_run_id != run_id:
            ...
            self.current_stock_code = None
            self.current_trade_date = None
            self.current_trades = self.app_manager.get_backtest_trades(run_id)
    ...
    def load_performance_data(self, run_id: int) -> pd.DataFrame:
        return self.app_manager.get_backtest_performance(run_id)
        
    def load_traded_stocks_summary(self, run_id: int) -> pd.DataFrame:
        return self.app_manager.get_traded_stocks_summary(run_id)

    def load_daily_chart_data(self, stock_code: str) -> tuple:
        if not self.current_run_id: return pd.DataFrame(), pd.DataFrame(), {}
        run_info = self.all_backtest_runs[self.all_backtest_runs['run_id'] == self.current_run_id]
        if run_info.empty: return pd.DataFrame(), pd.DataFrame(), {}
    ...
    def load_minute_chart_data(self, stock_code: str, trade_date: date) -> tuple:
        if not self.current_run_id or not trade_date: return pd.DataFrame(), pd.DataFrame(), {}
    ...
    def search_backtest_runs(self, search_text: str) -> pd.DataFrame:
        if not search_text: return self.all_backtest_runs
