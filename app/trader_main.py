...
from app.trader_view import TraderView
from app.trader_controller import TraderController
...
def main():
    ...
    try:
        ...
        all_runs_df = model.load_all_trader_runs(progress_callback=update_progress)
        # 전략 파라미터 파싱이 제거되어 70%에서 바로 완료
        # progress.setValue(70)  # 이미 load_all_trader_runs에서 70%로 설정됨
        app.processEvents()
        # --------------------------------------
        ...
        app.processEvents()
        controller = TraderController(view, model)
