...
import time
import sys
import os # sys.path.append를 위해 os 모듈 임포트

# 현재 스크립트의 경로를 sys.path에 추가하여 모듈 임포트 가능하게 함
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

from api.creon_api import CreonAPIClient
from backtest.backtester_daily_minute import Backtester
...
# --- 로깅 설정 ---
# 루트 로거 설정 (스크립트 전역에 적용)
logging.basicConfig(level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s',
                    datefmt='%Y-%m-%d %H:%M:%S',
                    handlers=[ # 로깅 핸들러 추가: 파일과 콘솔에 동시에 출력
                        logging.FileHandler("backtest_run.log"), # 백테스트 로그 파일
...
if __name__ == '__main__':
    ...
    # 백테스트 기간 설정
    # 일봉 데이터는 모멘텀 계산 기간 + 백테스트 기간만큼 필요
    ...
    # 안전자산 코드도 미리 추가
    all_target_stock_names = stock_names + [creon_api_client.get_stock_name(backtester_instance.strategy_params['safe_asset_code'])]
