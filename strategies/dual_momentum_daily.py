...
class DualMomentumDaily(DailyStrategy): # DailyStrategy 상속 
    ...
    def run_daily_logic(self, current_daily_date):
        """
        듀얼 모멘텀 스타일로 리팩터링: 모멘텀 스코어 계산 → 상위 N개 종목 선정 → _generate_signals → _log_rebalancing_summary 호출
        수정: 전일 데이터까지만 사용하여 장전 판단이 가능하도록 함
        """
        logging.info(f"{current_daily_date} - --- 일간 듀얼 모멘텀 로직 실행 중 (전일 데이터 기준) ---")
        ...
        if prev_trading_day is None:
            logging.warning(f"{current_daily_date}: 전일 데이터를 찾을 수 없어 듀얼 모멘텀 전략을 건너뜁니다.")
            return

        # 1. 모든 종목의 모멘텀 스코어 계산 (전일 데이터 기준)
        momentum_scores = {}
        for stock_code in self.data_store['daily']:
        ...
        # 5. 신호 생성 및 업데이트 (부모 클래스 메서드 사용) - 전일 데이터 기준
