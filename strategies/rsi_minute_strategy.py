...
import logging
import pandas as pd
import numpy as np # _calculate_rsi에서 필요할 수 있음
# from collections import defaultdict
# from typing import Union, List, Dict # 필요한 타입 힌트 임포트 (Python 3.7 호환)
#from strategies.strategy_base import Strategy
# Backtester 클래스에서 필요한 메서드를 호출하기 위해 RSIMinute 생성 시 Backtester 인스턴스를 전달받음
from util.utils import calculate_momentum, calculate_rsi # Assuming utils.py is in the same directory
...
class RSIMinute:
    def __init__(self, data_store, strategy_params, broker, position_info):
        ...
        self.position_info = position_info # Reference to Backtester's position_info (for stop-loss)
        ...
        if self.broker.positions and self._check_portfolio_stop_loss(current_prices_for_portfolio_check):
            ...
            for code, pos in list(self.broker.positions.items()): # 리스트로 복사하여 순회 중 딕셔너리 변경 허용
                if pos['size'] > 0:
                    ...
                    if code in self.momentum_signals:
