...
class RSIMinute(MinuteStrategy): # MinuteStrategy 상속 유지
    ...
    # _get_bar_at_time 메서드 삭제 (BaseStrategy로 이동)
    ...
    def run_minute_logic(self, stock_code, current_dt):
        ...
        current_minute_bar = self._get_bar_at_time('minute', stock_code, current_dt) # BaseStrategy의 메서드 사용
        ...
        for code in list(self.broker.positions.keys()): 
            if code != stock_code: # 현재 처리 중인 종목이 아닌 다른 보유 종목
                price_data = self._get_bar_at_time('minute', code, current_dt) # BaseStrategy의 메서드 사용
                if price_data is not None:
                    current_prices_for_portfolio_check[code] = price_data['close']
                else: # 분봉 데이터가 없으면 일봉 마지막 가격이라도 사용 (정확도는 떨어지지만 없는 것보다 낫다)
                    daily_price = self._get_bar_at_time('daily', code, current_dt.date()) # BaseStrategy의 메서드 사용
        ...
        minute_historical_data = self._get_historical_data_up_to('minute', stock_code, current_dt, lookback_period=required_rsi_data_len) # BaseStrategy의 메서드 사용
