...
from config.settings import INITIAL_CASH, STRATEGY_CONFIGS, OPTIMIZATION_LOOKBACK_MONTHS
...
def _run_single_strategy_optimization_worker(args: Dict) -> List[Dict]:
    ...
    try:
        # --- ▼ [수정] 최적화용 소수 유니버스(20개)를 사용하도록 변경 ---
        optimizer_universe = backtest_manager.get_optimizer_universe(optim_start, optim_end, 20)
        if not optimizer_universe:
            logger.warning(f"'{strategy_name}'의 최적화용 유니버스가 없어 건너뜁니다.")
            return []
        ...
        champion_params_by_regime = optimizer.run_optimization_for_strategy(
            ...
            custom_universe=optimizer_universe # 소수 유니버스 전달
        )
        # --- ▲ 수정 완료 ---
...
class WalkForwardOptimizer:
    ...
    def run_in_sample_profiling_step(self, train_start: date, train_end: date, model_id: int, model_name: str) -> bool:
        try:
            ...
            # 1. 최적화 기간을 학습 기간의 마지막 N개월로 설정
            optim_end_date = train_end
            
            # --- ▼ [2. 수정] 하드코딩된 숫자 대신 설정값(OPTIMIZATION_LOOKBACK_MONTHS)을 사용합니다. ---
            # timedelta를 사용한 계산 방식
            start_month = optim_end_date.month - (OPTIMIZATION_LOOKBACK_MONTHS - 1)
