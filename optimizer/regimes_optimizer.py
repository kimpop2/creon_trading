...
class RegimesOptimizer:
    ...
    def run_optimization_for_strategy(self, strategy_name: str, start_date: date, end_date: date, 
                                      regime_map: pd.DataFrame, model_name: str,
                                      custom_universe: Optional[List[str]] = None) -> Dict[int, Any]:
        logger.info(f"'{strategy_name}' 전략에 대한 국면별 최적화를 시작합니다 (Model: {model_name}).")
        # --- ▼ [수정] custom_universe를 HMMBacktest에 전달할 준비 ---
        # HMMBacktest의 prepare_for_system이 custom_universe를 받도록 수정 필요
        self.backtest.set_custom_universe(custom_universe)
