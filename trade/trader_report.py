...
class TraderReport:
    ...
    def generate_and_save_report(self,
        ...
        # run_id 생성 (DBManager에서 생성하도록 변경될 수 있음)
        ...
        run_id = self.db_manager.save_trader_run(run_data) # db_manager에서 run_id를 반환하지 않고, 여기서 생성하여 넘겨줌
        ...
        if trade_records:
            self.db_manager.save_trader_trade(trade_records)
        ...
        for date, value in portfolio_value_series.items():
            ...
            temp_portfolio_series = portfolio_value_series[:date]
        ...
        if performance_records:
            self.db_manager.save_trader_performance(performance_records)
