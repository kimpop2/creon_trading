...
class Backtest:
    ...
    def _update_daily_data_from_minute_bars(self, current_date: datetime.date):
        """
        매분 현재 날짜의 1분봉 데이터를 집계하여 일봉 데이터를 업데이트합니다.
        """
        for stock_code in self.data_store['daily'].keys():
            ...
            if minute_data_for_today is not None:
                # 현재 날짜에 해당하는 모든 1분봉 데이터 필터링
                # .copy()를 사용하여 SettingWithCopyWarning 방지
                today_minute_bars = minute_data_for_today[minute_data_for_today.index.date == current_date].copy()

                if not today_minute_bars.empty:
                    # 오늘의 일봉 데이터 계산
                    daily_open = today_minute_bars.iloc[0]['open']
                    daily_high = today_minute_bars['high'].max()
                    daily_low = today_minute_bars['low'].min()
                    daily_close = today_minute_bars.iloc[-1]['close'] # 마지막 1분봉의 종가
                    daily_volume = today_minute_bars['volume'].sum()

                    # 새로운 일봉 데이터 생성 (Series 형태)
                    new_daily_bar = pd.Series({
                        ...
                        'close': daily_close,
                        'volume': daily_volume
                    }, name=pd.Timestamp(current_date)) # 인덱스는 현재 날짜의 Timestamp

                    # self.data_store['daily'][stock_code] DataFrame 업데이트 또는 추가
                    # loc[]를 사용하면 인덱스가 존재하면 업데이트, 없으면 추가됩니다.
                    self.data_store['daily'][stock_code].loc[pd.Timestamp(current_date)] = new_daily_bar
                    # logging.debug(f"[{current_date}] {stock_code}: Daily bar updated - Close: {daily_close}")
    ...
    def run(self, start_date: datetime.date, end_date: datetime.date):
        ...
        self.portfolio_values.append((current_date - timedelta(days=1), initial_portfolio_value))
        ...
        for current_date in trading_dates:  
            ...
            # stocks = self.db_manager.fetch_daily_theme_stock(current_date - timedelta(days=20), prev_date)
            # #print(stocks)
            ...
            if self.minute_strategy:
                ...
                if not (has_trading_signals or has_stop_loss):
                    ...
                    if is_open_minute_strategy:
                        ...
                        trade_time = datetime.combine(current_date, time(9, 1))
                    ...
                    # 일반 분봉 전략 실행
                        ...
                        trade_time = datetime.combine(current_date, start_time) # 날짜+분

                        while trade_time <= datetime.combine(current_date, end_time):
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 듀얼 모멘텀 전략 설정 (DualMomentumDaily 인스턴스 생성 및 Backtest에 주입)
        dual_daily_params={
        ...
        open_minute_params={
            ...
            'num_top_stocks': 10,            # 일봉 전략과 동일한 값으로 설정
        }
        ...
        stop_loss_params = {
            ...
            'stop_loss_ratio': -5,        # 매수가 기준 손절율: -6.0%
            'trailing_stop_ratio': -3,    # 최고가 기준 트레일링 손절률: -4.0%
