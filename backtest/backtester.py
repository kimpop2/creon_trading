...
from backtest.broker import Broker # Assuming broker.py is in the same directory
from strategies.dual_momentum_daily import DualMomentumDaily
from strategies.rsi_minute import RSIMinute 
from util.utils import calculate_performance_metrics, get_next_weekday # Assuming utils.py is in the same directory
...
class Backtester:
    def __init__(self, creon_api_client, initial_cash=10_000_000, commission_rate=0.00015):
        ...
        # 전략 클래스 인스턴스화
        self.daily_strategy = DualMomentumDaily(self.data_store, self.strategy_params, self.broker)
        self.minute_strategy = RSIMinute(self.data_store, self.strategy_params, self.broker, self.position_info)
