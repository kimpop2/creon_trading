...
class CreonAPIClient:
    ...
    def get_unexecuted_orders(api_client, stock_code: str):
        # Retrieve all unfilled orders
        all_unfilled_orders = api_client.get_unfilled_orders()
        
        # Filter the list to include only orders matching the specific stock_code
        filtered_orders = [order for order in all_unfilled_orders if order.get('stock_code') == stock_code]
