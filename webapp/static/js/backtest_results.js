    ...
    async function handleStockSelected(stockCode, stockName) {
        ...
        try {
            ...
            createOrUpdateChart('dailyChart', {
                ...
                data: { 
                    datasets: [
                        { label: 'OHLC', data: ohlc }, 
                        { label: '매수', type: 'scatter', data: buySignals, backgroundColor: 'green', pointStyle: 'triangle', radius: 7, rotation: 0 }, 
                        { label: '매도', type: 'scatter', data: sellSignals, backgroundColor: 'red', pointStyle: 'triangle', radius: 7, rotation: 180 }, 
                        { label: '거래량', type: 'bar', data: volume, yAxisID: 'yVolume' }
                    ] 
                },
                options: { 
                    scales: { 
                    ...
                    onClick: (event, elements) => {
                        if (elements.length > 0) {
                            ...
                            // 클릭된 요소의 데이터셋에서 직접 x값(타임스탬프)을 가져옵니다.
                            const clickedTimestamp = chart.data.datasets[0].data[dataIndex].x;
                            
                            // 타임스탬프를 Date 객체로 변환 후 포맷팅하여 전달합니다.
                            handleDateSelected(formatDate(new Date(clickedTimestamp)));
                        }
                    }
                }
            });
