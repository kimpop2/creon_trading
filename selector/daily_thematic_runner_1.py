...
def process_daily_theme_data(cursor, connection, stock_names_set, stock_to_themes_map,
                             data_period_days=20): # data_period_days 파라미터 유지
    """
    ...
    try:
        ...
        for theme_class_name, words_data in theme_word_aggr.items():
            total_theme_relevance = 0.0
            total_avg_rate_for_theme = 0.0
            num_words_in_theme = 0

            for word, metrics in words_data.items():
                total_theme_relevance += metrics['count'] * (1 + abs(metrics['sum_rate'] / metrics['count'] / 100.0)) if metrics['count'] > 0 else 0
                total_avg_rate_for_theme += metrics['sum_rate'] / metrics['count'] if metrics['count'] > 0 else 0
                num_words_in_theme += 1
            
            if num_words_in_theme > 0:
                # 단순한 momentum_score: 테마 관련 단어들의 총 관련성 점수
                momentum_score = total_theme_relevance 
...
def run_daily_analysis_and_get_actionable_insights(conn, data_period_days=180): # data_period_days 파라미터 추가
...
if __name__ == "__main__":
    ...
    test_data_period_days = 30 # 최근 트렌드를 위해 기간을 줄였습니다.
