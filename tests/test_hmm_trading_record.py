# tests/test_brokerage_trading.py
...
from manager.trading_manager import TradingManager # TradingManager 임포트
...
class TestBrokerageTradeRecording(unittest.TestCase):
    ...
    def test_trade_conclusion_saves_with_strategy_name(self, mock_api_client, mock_manager):
        ...
        # 3. Assert (검증)
        # manager를 통해 db_manager의 save_trading_trade가 호출되었는지 확인
        mock_db_manager = mock_manager.db_manager
        
        # 한 번 호출되었는지 확인
        mock_db_manager.save_trading_log.assert_called_once()
        
        # 호출될 때 전달된 인자(kwargs)를 가져옴
        call_args, call_kwargs = mock_db_manager.save_trading_trade.call_args
        saved_data = call_args[0]
        
        # strategy_name이 정확히 전달되었는지 검증
