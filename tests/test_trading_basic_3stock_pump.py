# tests/test_trading_basic_3stocks_pump.py
...
class TestCreonAPIScenario(unittest.TestCase):
    ...
    def _conclusion_callback(cls, data: Dict[str, Any]):
        ...
        with cls._callback_lock:
            ...
            if not is_duplicate_log:
                logger.info(f"[CpEvent] 주문 체결/응답 수신: {data.get('flag')} {data.get('buy_sell')} 종목:{data.get('code')} 가격:{data.get('price'):,.0f} 수량:{data.get('quantity')} 주문번호:{data.get('order_num')} 잔고:{data.get('balance')}")
    ...
    def _execute_trading_scenario(self, stock_code: str, order_quantity: int):
        ...
        sell_order_result = api.send_order(
            ...
            quantity=sellable_quantity,  # 매도 가능 수량 전체 매도 시도
